    ...
    override suspend fun runTasksInternal(operation: ApmOperation) {
        do {
            if (operation.chainMonitor.isReady() && nodeCoreLiteKit.network.isReady()) {
                ...
                logger.info(operation, "Unable to continue with the task 'Retrieve Mining Instruction': ${getExtraMinerStatusInformation(operation.chain.name, operation.chainMonitor)}")
                delay(10 * 1000)
            }
        } while (operation.state != ApmOperationState.INSTRUCTION)
        ...
        do {
            if (operation.chainMonitor.isReady() && nodeCoreLiteKit.network.isReady()) {
                ...
                logger.info(operation, "Unable to continue with the task 'Create Endorsement Transaction': ${getExtraMinerStatusInformation(operation.chain.name, operation.chainMonitor)}" )
                delay(10 * 1000)
            }
        } while (operation.state != ApmOperationState.ENDORSEMENT_TRANSACTION)
        ...
        do {
            if (operation.chainMonitor.isReady() && nodeCoreLiteKit.network.isReady()) {
                ...
                logger.info(operation, "Unable to continue with the task 'Confirm transaction': ${getExtraMinerStatusInformation(operation.chain.name, operation.chainMonitor)}")
                delay(10 * 1000)
            }
        } while (operation.state != ApmOperationState.ENDORSEMENT_TX_CONFIRMED)
        ...
        do {
            if (operation.chainMonitor.isReady() && nodeCoreLiteKit.network.isReady()) {
                ...
                logger.info(operation, "Unable to continue with the task 'Determine Block of Proof': ${getExtraMinerStatusInformation(operation.chain.name, operation.chainMonitor)}")
                delay(10 * 1000)
            }
        } while (operation.state != ApmOperationState.BLOCK_OF_PROOF)
        ...
        do {
            if (operation.chainMonitor.isReady() && nodeCoreLiteKit.network.isReady()) {
                ...
                logger.info(operation, "Unable to continue with the task 'Prove Transaction': ${getExtraMinerStatusInformation(operation.chain.name, operation.chainMonitor)}")
                delay(10 * 1000)
            }
        } while (operation.state != ApmOperationState.PROVEN)
        ...
        do {
            if (operation.chainMonitor.isReady() && nodeCoreLiteKit.network.isReady()) {
                ...
                logger.info(operation, "Unable to continue with the task 'Wait for next VeriBlock Keystone': ${getExtraMinerStatusInformation(operation.chain.name, operation.chainMonitor)}")
                delay(10 * 1000)
            }
        } while (operation.state != ApmOperationState.KEYSTONE_OF_PROOF)
        ...
        do {
            if (operation.chainMonitor.isReady() && nodeCoreLiteKit.network.isReady()) {
                ...
                logger.info(operation, "Unable to continue with the task 'Retrieve VeriBlock Publication data': ${getExtraMinerStatusInformation(operation.chain.name, operation.chainMonitor)}")
                delay(10 * 1000)
            }
        } while (operation.state != ApmOperationState.CONTEXT)
        ...
        do {
            if (operation.chainMonitor.isReady() && nodeCoreLiteKit.network.isReady()) {
                ...
                logger.info(operation, "Unable to continue with the task 'Submit Proof of Proof': ${getExtraMinerStatusInformation(operation.chain.name, operation.chainMonitor)}")
                delay(10 * 1000)
            }
        } while (operation.state != ApmOperationState.SUBMITTED_POP_DATA)
        ...
        do {
            if (operation.chainMonitor.isReady() && nodeCoreLiteKit.network.isReady()) {
                ...
                logger.info(operation, "Unable to continue with the task 'Confirm PoP Transaction': ${getExtraMinerStatusInformation(operation.chain.name, operation.chainMonitor)}")
                delay(10 * 1000)
            }
        } while (operation.state != ApmOperationState.POP_TX_CONFIRMED)
        ...
        do {
            if (operation.chainMonitor.isReady() && nodeCoreLiteKit.network.isReady()) {
                ...
                logger.info(operation, "Unable to continue with the task 'Payout Detection': ${getExtraMinerStatusInformation(operation.chain.name, operation.chainMonitor)}")
                delay(10 * 1000)
            }
        } while (operation.state != MiningOperationState.COMPLETED)
    }
