...
class VeriBlockBlockStore(
     ...
     */
    protected var notFoundCache: LinkedHashMap<AnyVbkHash, Any> = object : LinkedHashMap<AnyVbkHash, Any>() {
        override fun removeEldestEntry(eldest: MutableMap.MutableEntry<AnyVbkHash, Any>?): Boolean {
    ...
    fun getChainHead(): StoredVeriBlockBlock? {
        ...
        return lock.withLock {
            if (lastChainHead == null) {
                val headHash = ByteArray(VbkHash.HASH_LENGTH)
                buffer.position(8)
                buffer.get(headHash)
                val hash = headHash.asAnyVbkHash()
                if (VbkHash.EMPTY_HASH == hash) {
    ...
    fun put(storedBlock: StoredVeriBlockBlock): StoredVeriBlockBlock {
        ...
        return lock.withLock {
            ...
            buffer.position(cursor + VbkHash.HASH_LENGTH)
    ...
    fun get(hash: AnyVbkHash): StoredVeriBlockBlock? {
        ...
        return lock.withLock {
            ...
            val offset = VbkHash.HASH_LENGTH - targeVBlakeHashBytes.size
    ...
    @Throws(BlockStoreException::class)
    fun get(hash: AnyVbkHash, count: Int): List<StoredVeriBlockBlock> {
        val buffer = safeBuffer
        ...
        return lock.withLock {
            ...
            var offset = VbkHash.HASH_LENGTH - targeVBlakeHashBytes.size
            ...
            do {
                ...
                if (Arrays.equals(scratch, targeVBlakeHashBytes)) {
                    ...
                    if (!foundInitial) {
                        ...
                        offset = VbkHash.HASH_LENGTH - targeVBlakeHashBytes.size
    ...
    fun reset() {
        ...
        lock.withLock {
            ...
            setRingCursor(buffer!!, FILE_PROLOGUE_BYTES)
            buffer!!.position(8)
            buffer!!.put(VbkHash.EMPTY_HASH.bytes)
            lastChainHead = null
        }
    }
}
