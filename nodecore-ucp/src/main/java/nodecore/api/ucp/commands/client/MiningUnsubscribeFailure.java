...
public class MiningUnsubscribeFailure extends UCPClientCommand {
    private final UCPCommand.Command command = Command.MINING_UNSUBSCRIBE_FAILURE; // Not static for serialization purposes

    // Required
    private final UCPArgumentRequestID request_id;
    private final UCPArgumentMessage reason;

    public MiningUnsubscribeFailure(UCPArgument... arguments) {
    ...
    public MiningUnsubscribeFailure(int request_id, String reason) {
        this.request_id = new UCPArgumentRequestID(request_id);
        this.reason = new UCPArgumentMessage(reason);
    }

    public static MiningUnsubscribeFailure reconstitute(String commandLine) {
        if (commandLine == null) {
        ...
        deserializer.registerTypeAdapter(MiningUnsubscribeFailure.class, new MiningUnsubscribeFailureDeserializer());
        return deserializer.create().fromJson(commandLine, MiningUnsubscribeFailure.class);
    }
