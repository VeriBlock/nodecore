...
public class ErrorUnknownCommand extends UCPClientCommand {
    private final UCPCommand.Command command = Command.ERROR_UNKNOWN_COMMAND; // Not static for serialization purposes

    // Required
    private final UCPArgumentRequestID request_id;

    public ErrorUnknownCommand(UCPArgument ... arguments) {
    ...
    public ErrorUnknownCommand(int request_id) {
        this.request_id = new UCPArgumentRequestID(request_id);
    }

    public static ErrorUnknownCommand reconstitute(String commandLine) {
        if (commandLine == null) {
        ...
        deserializer.registerTypeAdapter(ErrorUnknownCommand.class, new ErrorUnknownCommandDeserializer());
        return deserializer.create().fromJson(commandLine, ErrorUnknownCommand.class);
    }
