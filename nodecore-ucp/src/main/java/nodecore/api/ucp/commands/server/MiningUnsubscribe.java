...
public class MiningUnsubscribe extends UCPServerCommand {
    private final UCPCommand.Command command = Command.MINING_UNSUBSCRIBE; // Not static for serialization purposes

    // Required
    private final UCPArgumentRequestID request_id;


    public MiningUnsubscribe(UCPArgument ... arguments) {
    ...
    public MiningUnsubscribe(int request_id) {
        this.request_id = new UCPArgumentRequestID(request_id);
    }

    public static MiningUnsubscribe reconstitute(String commandLine) {
        if (commandLine == null) {
        ...
        deserializer.registerTypeAdapter(MiningUnsubscribe.class, new MiningUnsubscribeDeserializer());
        return deserializer.create().fromJson(commandLine, MiningUnsubscribe.class);
    }
