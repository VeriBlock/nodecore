...
public class ErrorTransactionInvalid extends UCPClientCommand {
    private final UCPCommand.Command command = Command.ERROR_TRANSACTION_INVALID; // Not static for serialization purposes

    // Required
    private final UCPArgumentRequestID request_id;
    private final UCPArgumentTransactionData transaction_data;

    public ErrorTransactionInvalid(UCPArgument ... arguments) {
        ...
        this.transaction_data = (UCPArgumentTransactionData)arguments[1];
    }

    public ErrorTransactionInvalid(int request_id, String transaction_data) {
        this.request_id = new UCPArgumentRequestID(request_id);
        this.transaction_data = new UCPArgumentTransactionData(transaction_data);
    }

    public static ErrorTransactionInvalid reconstitute(String commandLine) {
        if (commandLine == null) {
        ...
        deserializer.registerTypeAdapter(ErrorTransactionInvalid.class, new ErrorTransactionInvalidDeserializer());
        return deserializer.create().fromJson(commandLine, ErrorTransactionInvalid.class);
    }
    ...
    public String getTransactionData() {
        return transaction_data.getData();
    }
