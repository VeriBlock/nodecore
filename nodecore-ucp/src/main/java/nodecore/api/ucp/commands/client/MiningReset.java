...
public class MiningReset extends UCPClientCommand {
    private final UCPCommand.Command command = Command.MINING_RESET; // Not static for serialization purposes

    // Required
    private final UCPArgumentRequestID request_id;

    public MiningReset(UCPArgument ... arguments) {
    ...
    public MiningReset(int request_id) {
        this.request_id = new UCPArgumentRequestID(request_id);
    }

    public static MiningReset reconstitute(String commandLine) {
        if (commandLine == null) {
        ...
        deserializer.registerTypeAdapter(MiningReset.class, new MiningResetDeserializer());
        return deserializer.create().fromJson(commandLine, MiningReset.class);
    }
