...
public class MiningSubscribeFailure extends UCPClientCommand {
    private final UCPCommand.Command command = Command.MINING_SUBSCRIBE_FAILURE; // Not static for serialization purposes

    // Required
    private final UCPArgumentRequestID request_id;
    private final UCPArgumentMessage reason;

    public MiningSubscribeFailure(UCPArgument ... arguments) {
    ...
    public MiningSubscribeFailure(int request_id, String reason) {
        this.request_id = new UCPArgumentRequestID(request_id);
        this.reason = new UCPArgumentMessage(reason);
    }

    public static MiningSubscribeFailure reconstitute(String commandLine) {
        if (commandLine == null) {
        ...
        deserializer.registerTypeAdapter(MiningSubscribeFailure.class, new MiningSubscribeFailureDeserializer());
        return deserializer.create().fromJson(commandLine, MiningSubscribeFailure.class);
    }
