...
public class MiningAuthFailure extends UCPClientCommand {
    private final UCPCommand.Command command = Command.MINING_AUTH_FAILURE; // Not static for serialization purposes

    // Required
    private final UCPArgumentRequestID request_id;
    private final UCPArgumentMessage reason;

    public MiningAuthFailure(UCPArgument ... arguments) {
        ...
        this.reason = (UCPArgumentMessage)arguments[0];
    }

    public MiningAuthFailure(int request_id, String reason) {
        this.request_id = new UCPArgumentRequestID(request_id);
        this.reason = new UCPArgumentMessage(reason);
    }

    public static MiningAuthFailure reconstitute(String commandLine) {
        if (commandLine == null) {
        ...
        deserializer.registerTypeAdapter(MiningAuthFailure.class, new MiningAuthFailureDeserializer());
        return deserializer.create().fromJson(commandLine, MiningAuthFailure.class);
    }
