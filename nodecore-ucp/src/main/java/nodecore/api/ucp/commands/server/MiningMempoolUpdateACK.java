...
public class MiningMempoolUpdateACK extends UCPServerCommand {
    private final UCPCommand.Command command = Command.MINING_MEMPOOL_UPDATE_ACK; // Not static for serialization purposes

    // Required
    private final UCPArgumentRequestID request_id;


    public MiningMempoolUpdateACK(UCPArgument ... arguments) {
    ...
    public MiningMempoolUpdateACK(int request_id) {
        this.request_id = new UCPArgumentRequestID(request_id);
    }

    public static MiningMempoolUpdateACK reconstitute(String commandLine) {
        if (commandLine == null) {
        ...
        deserializer.registerTypeAdapter(MiningMempoolUpdateACK.class, new MiningMempoolUpdateACKDeserializer());
        return deserializer.create().fromJson(commandLine, MiningMempoolUpdateACK.class);
    }
