...
public class MiningResetACK extends UCPServerCommand {
    private final UCPCommand.Command command = Command.MINING_RESET_ACK; // Not static for serialization purposes

    // Required
    private final UCPArgumentRequestID request_id;


    public MiningResetACK(UCPArgument ... arguments) {
    ...
    public MiningResetACK(int request_id) {
        this.request_id = new UCPArgumentRequestID(request_id);
    }

    public static MiningResetACK reconstitute(String commandLine) {
        if (commandLine == null) {
        ...
        deserializer.registerTypeAdapter(MiningResetACK.class, new MiningResetACKDeserializer());
        return deserializer.create().fromJson(commandLine, MiningResetACK.class);
    }
