...
public class MiningAuthSuccess extends UCPClientCommand {
    private final UCPCommand.Command command = Command.MINING_AUTH_SUCCESS; // Not static for serialization purposes

    // Required
    private final UCPArgumentRequestID request_id;

    public MiningAuthSuccess(UCPArgument ... arguments) {
    ...
    public MiningAuthSuccess(int request_id) {
        this.request_id = new UCPArgumentRequestID(request_id);
    }

    public static MiningAuthSuccess reconstitute(String commandLine) {
        if (commandLine == null) {
        ...
        deserializer.registerTypeAdapter(MiningAuthSuccess.class, new MiningAuthSuccessDeserializer());
        return deserializer.create().fromJson(commandLine, MiningAuthSuccess.class);
    }
