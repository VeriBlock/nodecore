...
public class ErrorRangeNotAvailable extends UCPClientCommand {
    private final UCPCommand.Command command = UCPCommand.Command.ERROR_RANGE_NOT_AVAILABLE; // Not static for serialization purposes

    // Required
    private final UCPArgumentRequestID request_id;

    public ErrorRangeNotAvailable(UCPArgument ... arguments) {
    ...
    public ErrorRangeNotAvailable(int request_id) {
        this.request_id = new UCPArgumentRequestID(request_id);
    }

    public static ErrorRangeNotAvailable reconstitute(String commandLine) {
        if (commandLine == null) {
        ...
        deserializer.registerTypeAdapter(ErrorRangeNotAvailable.class, new ErrorRangeNotAvailableDeserializer());
        return deserializer.create().fromJson(commandLine, ErrorRangeNotAvailable.class);
    }
