...
import nodecore.api.ucp.storage.DeltaList;

public class UCPArgumentTransactionDeltaList extends UCPArgument {

    private final UCPArgument.UCPType type = UCPType.TRANSACTION_DELTA_LIST;
     ...
     * @param data
     */
    public UCPArgumentTransactionDeltaList(String data) {
        if (data == null) {
        ...
        // Test to see if valid DeltaList argument
        new DeltaList(data);
    }

    public UCPArgumentTransactionDeltaList(DeltaList list) {
        this(list.serialize());
    }
     ...
     */
    public DeltaList getData() {
        return new DeltaList(data);
    }
