    ...
    public static void progPowLoop(
        ...
        for (int l = 0; l < PROGPOW_LANES; l++) {
            ...
            for (int i = 0; i < PROGPOW_DAG_LOADS; i++) {
                ...
                dag_entry[l][i] = (lookupHolder[lookupOffset] & 0x000000FF) |
                    ((lookupHolder[lookupOffset + 1] & 0x000000FF) << 8) |
                    ((lookupHolder[lookupOffset + 2] & 0x000000FF) << 16) |
                    ((lookupHolder[lookupOffset + 3] & 0x000000FF) << 24);
            }
