...
public class Sha256Hash implements Comparable<Sha256Hash> {
    ...
    private Sha256Hash(byte[] rawHashBytes, int length) {
        ...
        Preconditions.argument(rawHashBytes.length == length, () -> "Invalid Sha256 hash: " + Utility.bytesToHex(rawHashBytes));
        this.length = length;
    ...
    public static Sha256Hash wrap(String hexString) {
        return wrap(Utility.hexToBytes(hexString));
    }
    ...
    public static Sha256Hash wrap(String hexString, int length) {
        return wrap(Utility.hexToBytes(hexString), length);
    }
    ...
    public static Sha256Hash wrapReversed(byte[] rawHashBytes) {
        return wrap(Utility.flip(rawHashBytes));
    }
    ...
    public static Sha256Hash wrapReversed(byte[] rawHashBytes, int length) {
        return wrap(Utility.flip(rawHashBytes), length);
    }
    ...
    public static Sha256Hash wrapReversed(String hexString) {
        return wrap(Utility.flip(Utility.hexToBytes(hexString)));
    }
    ...
    public byte[] getReversedBytes() {
        return Utility.flip(bytes);
    }

    public Sha256Hash getReversed() {
        return new Sha256Hash(Utility.flip(bytes), bytes.length);
    }
