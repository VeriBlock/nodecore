...
public class StratumArgumentPort extends StratumArgument {

    private final StratumArgument.StratumType type = StratumType.PORT;
     ...
     * @param data
     */
    public StratumArgumentPort(String data) {
        if (data == null) {
        ...
        if (this.data > 65535) {
            throw new IllegalArgumentException(getClass().getCanonicalName() + "'s constructor cannot be called with an invalid port (" +
                this.data + ")!");
        }
    }
     ...
     * @param data
     */
    public StratumArgumentPort(Integer data) {
