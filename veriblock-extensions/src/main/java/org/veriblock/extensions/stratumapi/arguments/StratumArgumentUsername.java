...
public class StratumArgumentUsername extends StratumArgument {

    private final StratumArgument.StratumType type = StratumType.USERNAME;
     ...
     * @param data
     */
    public StratumArgumentUsername(String data) {
        if (data == null) {
        ...
        // A username is not required to be an address during solo mining (or by the stratum protocol).
        // Checks to ensure that a miner uses a valid payout address happen in the pool software if/when running in standard (non-solo) mode.
