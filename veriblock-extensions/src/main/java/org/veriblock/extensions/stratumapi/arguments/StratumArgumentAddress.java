...
public class StratumArgumentAddress extends StratumArgument {

    private final StratumArgument.StratumType type = StratumType.ADDRESS;
     ...
     * @param data
     */
    public StratumArgumentAddress(String data) {
        if (data == null) {
        ...
        if (!AddressUtility.isValidStandardOrMultisigAddress(data)) {
            throw new IllegalArgumentException(getClass().getCanonicalName() + "'s constructor cannot be called with an invalid address (" +
                data +
                ")!");
        }
