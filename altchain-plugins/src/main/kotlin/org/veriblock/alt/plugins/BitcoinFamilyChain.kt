...
class BitcoinFamilyChain(
    ...
    override fun getPublicationData(blockHeight: Int?): PublicationDataWithContext {
        ...
        val contextInfoBuffer = ByteBuffer.allocate(100)
        contextInfoBuffer.putInt(actualBlockHeight)
        val publicationData = PublicationData(
            getChainIdentifier(),
            response.block_header.asHex(),
            Base58.decode("VFMJSUgJCy9QRa1RjXNmJ5kLy5D35C"), // TODO retrieve from response
            response.raw_contextinfocontainer.asHex()
        )
        ...
        return PublicationDataWithContext(publicationData, response.last_known_veriblock_blocks.map { it.asHex() }, response.last_known_bitcoin_blocks.map { it.asHex() })
    }
