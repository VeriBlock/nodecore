...
class PreservedPoPMiningOperationStateTests {
    private var data: PopMiningOperationStateDto? = null

    @Before
    fun before() {
        data = PopMiningOperationStateDto().also {
            it.operationId = "6d2abb8d"
            it.status = OperationStatus.RUNNING
            it.currentAction = OperationStateType.CONFIRMED
            it.miningInstruction = PopMiningInstruction()
            it.miningInstruction.publicationData = Utility.hexToBytes(
            ...
            it.miningInstruction.endorsedBlockHeader = Utility.hexToBytes(
            ...
            it.miningInstruction.lastBitcoinBlock = Utility.hexToBytes(
            ...
            it.miningInstruction.minerAddress = Utility.hexToBytes("672A9D1281CC9D25BFB4539145702BFB2D8AE61DBBC0")
            it.transaction = Utility.hexToBytes(
            ...
            it.submittedTransactionId = "43c9b4c05a2f64d08a9ca0be7cdae61d4179cb2c77a00941145081a9aad1a8d4"
            it.bitcoinBlockHeaderOfProof = Utility.hexToBytes(
            ...
            it.bitcoinContextBlocks = ArrayList()
            it.bitcoinContextBlocks.add(
                Utility.hexToBytes(
            ...
            it.merklePath = "1:D4A8D1AAA98150144109A0772CCB79411DE6DA7CBEA09C8AD0642F5AC0B4C943:8D94793049965927A4DDCB3911C20EDE6895504075860112779442EAE0C28610"
            it.alternateBlocksOfProof = LinkedHashSet()
            it.alternateBlocksOfProof.add(
                Utility.hexToBytes(
    ...
    fun serializationWhenAllPropertiesSet() {
        val returned = serializeThenDeserialize(data)
    ...
    private fun serializeThenDeserialize(input: PopMiningOperationStateDto?): PopMiningOperationStateDto? {
        ...
        try {
            ByteArrayInputStream(serialized).use { dataIn ->
                ObjectInputStream(
                    dataIn
                ).use { `in` -> return `in`.readObject() as PopMiningOperationStateDto }
            }
