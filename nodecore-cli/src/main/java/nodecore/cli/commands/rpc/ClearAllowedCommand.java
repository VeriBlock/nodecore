...
import nodecore.api.grpc.ClearAllowedRequest;
...
public class ClearAllowedCommand implements Command {
    private static final Logger _logger = LoggerFactory.getLogger(ClearAllowedCommand.class);

    @Inject
    public ClearAllowedCommand() {
    }
    ...
    public Result execute(CommandContext context) {
        ...
        try {
            ProtocolReply reply = context
                    .adminService()
                    .clearAllowed(ClearAllowedRequest.newBuilder().build());
            if (!reply.getSuccess()) {
                result.fail();
            } else {
